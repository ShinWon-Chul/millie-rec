...
- **D-05 (2026-09-06 architect 검토로 정정):** 이 문서의 D-01~D-03·code_context 자수는 **UTF-8 바이트**를 자수로 잘못 적은 것이다(예 1장 "4,449" = 4,503바이트, 실제 문자 2,311). 실측 단위는 **한글 자수**(Python `re.findall(r'[가-힣]')`, 표·mermaid 텍스트 포함): 현재 1장 1,054 · 2장 1,065 · 3장 1,471 · 4장 1,371 · 5장 1,193(산문만 1,152). 규칙 상한은 장당 ≈1,000~1,100(`../.claude/rules/report.md`, 표 2~4·mermaid 1 별도). **감축 목표(실측 기준):** 3장 ≤1,100(≈−370) · 4장 ≤1,150(8행 표 텍스트 포함이라 +50 허용, ≈−250) · 5장은 새 요소(URL 박스·시나리오·클릭 경로·캡션 6·★박스 3·4층 표·5권 표)로 **순증하므로 §5-1·§5-2 문단 벽 삭제로 상쇄해 산문 한글 ≤1,100**, 표·캡션·그림 면적은 장 수로 본다. 1·2장은 유지. **최종 게이트는 사용자 Notion 장 수 회신(D-16) 하나**이고 계측은 안내 지표다. `/pdf-check` 6항 자수 검사는 **산문 기준**으로 판정하고 표·mermaid 포함 초과는 경고로 취급한다. **5장 초과 시 삭제 차순(5장 전용):** ① 캡처 캡션 6개를 각 1구절로 ② 난이도 4층 피처 표 → ★난이도 박스 안 1문장으로 흡수 ③ 시나리오 3문장 → 2문장 ④ 앵커 박스 4번째 문장 ⑤ 로드맵 1줄. **절대 보존:** ★3 박스(제목 + 핵심 1문장) · 5권 표 · 데모 URL 2곳·상태 페이지 · D-19 클릭 경로 · 기억할 5가지 · 그림 자리 표시 6개. 1~4장 차순은 기존대로(해석 문단 → 각주 부연 → 표 행 축약, 필수 문장 7·숫자 출처·mermaid 4 보존).
...
- `millie-rec/results/millie_coverage.csv` · `results/millie_difficulty.json` · `results/millie_edges_gate.json` — 카탈로그 9,447권·완독지수 82.5%·잔차 난이도 분야 평균 **0.488(어린이)~0.505(에세이/시), 8개 분야 전체 범위**(완독 확률 분야 평균은 40.3% 오디오북~79.0% 어린이 — draft 현행 "0.502~0.505"·"40.3% vs 54.8%"는 상위 4개만 고른 표기라 정정 대상, 플래너 발견 2026-09-06)·동일 카테고리 비율 0.356.
