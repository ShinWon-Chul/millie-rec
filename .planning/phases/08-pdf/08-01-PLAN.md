...
**1-c. H2 축 태그(`/pdf-check` 8-②, 형식은 Claude's Discretion → H2 끝에 `[①②]`처럼 붙임).** 1·2장 H2 6개를 다음으로 바꾼다(제목 본문은 유지, 끝에 태그만 추가 — 단 §2-1의 `(그림 1)`은 `(그림 2)`로, 이유는 1-e):
- `## 1-1. 한 줄 요약 [①]`
- `## 1-2. 문제 [①]`
...
**1-d. 1장 첫 문단에 4축 각 한 문장(`/pdf-check` 8-①, 문단 3문장 규칙을 지키기 위해 표로).** §1-1의 기존 blockquote(`> 취향 설정으로 시작하되 …`)는 그대로 두고, 그 바로 아래에 다음 표를 추가한다:
```
| 축 | 한 문장 | 장 |
|---|---|---|
...
**1-i. 마무리 검사.** 1·2장 범위(`# 3.` 이전)에서 `Phase [0-9]`·`Day [0-9]`·`freeze`·`구현 완료`·`make serve`·`[Phase`·`[재확인]`이 0건인지 확인한다. 2장의 `**구현 결정.** α/β/γ는 …` 문단은 유지한다(설계 설명이며 개발 과정 표기가 아님).
  ...
  <acceptance_criteria>
    ...
    - `grep -c '^| ① 추천 목표 |' report/draft.md` = 1 · `grep -c '^| ④ 성능 평가 |' report/draft.md` = 1 (4축 표)
    ...
    - 필수 문장 ①·② 보존: `grep -c 'CTR(Click-Through Rate: 클릭률)은 진단 지표이지 목적이 아니다' report/draft.md` ≥ 1 · `grep -c '취향 재설정 ≠ 프로필 초기화' report/draft.md` ≥ 1
...
**2-b. 필수 문장 ③ 본문 승격(결정 D-13).** 계층 표(`| **Online** | 요청 시 | …`) 아래 문단 `**계층 배치의 기준은 지연 예산이다.** 추천 API p95 200ms(…)를 단계별로 쪼갰고 … **추천 API 장애가 메인 장애가 되어선 안 된다.**`의 **첫 문장 앞**에 다음 문장을 추가한다(문단은 4문장이 되므로, 마지막 문장 `**추천 API 장애가 메인 장애가 되어선 안 된다.**`는 다음 문단(2-c의 구현 1문장)으로 옮겨 앞에 붙인다):
```
**요청마다 무거운 계산을 다시 하지 않는다.**
```
결과 문단 1(3문장): `**요청마다 무거운 계산을 다시 하지 않는다.** **계층 배치의 기준은 지연 예산이다.** 추천 API p95 200ms(과제용 제안 SLO(Service Level Objective: 서비스 수준 목표); 내부 값 비공개)를 단계별로 쪼갰고 예산을 넘는 연산은 전부 Offline·Nearline으로 내렸다. 예산을 넘거나 예외가 나면 fallback cascade(단계적 대체) 0→3으로 내려간다.` — 문장 수를 3으로 맞추기 위해 `**요청마다 …**`와 `**계층 배치의 기준은 지연 예산이다.**`는 한 문장으로 합쳐 `**요청마다 무거운 계산을 다시 하지 않는다** — 계층 배치의 기준은 지연 예산이다.`로 쓴다.
...
**2-f. 마무리 검사.** 3장 범위에서 `Phase [0-9]`·`Day [0-9]`·`freeze`·`구현 완료`·`make serve`·`5일`이 0건인지 확인한다. §3-2 표 열 제목 `5일 MVP (구현)`은 `MVP (구현)`으로 바꾼다.
  ...
  <acceptance_criteria>
    ...
    - `echo "$R" | grep -v '^#' | grep -c '요청마다 무거운 계산을 다시 하지 않는다'` ≥ 1 (본문 줄)
    - `echo "$R" | grep -c '^\*\*추천 API 장애가 메인 장애가 되어선 안 된다\.\*\* 파이프라인이 예외를 던져도 200 + 인기 행을 반환하는 것이 테스트로 존재한다'` = 1 · `echo "$R" | grep -c '구현 현황'` = 0
    - `echo "$R" | grep -c 'LLM(Large Language Model: 대규모 언어 모델)은 넣지 않았다'` = 1 · `grep -c 'Two-Tower는 넣지 않았다'` = 1 · `grep -c 'ALS(Alternating Least Squares: 행렬 분해 협업 필터링)'` = 1 — 세 문장이 같은 문단(연속 줄 1개)에 있다: `echo "$R" | grep -c '^\*\*의도적으로 넣지 않은 것\.\*\*'` = 1
...
```
같은 모델을 온보딩 직후(n=0)와 이력 축적(n≥20)에서 따로 측정한 이유는 §2-2 "가중치가 변해야 한다"를 숫자로 보이기 위해서다. hybrid는 n≥20에서 pop 대비 NDCG@10을 0.153 벌려(0.087→0.240) 이력 성분 β가 순위화에 들어온다는 주장의 실측 증거가 된다. pop 기준선의 두 값 차이는 추천 제외 범위(n=0은 5권만, n≥20은 이력 전체)에서 온 바닥선이라 모델 성능과 무관하다.
...
```
(영어 "freeze"는 쓰지 않는다. 숫자는 위 `<interfaces>`의 정본과 글자 단위로 같다: 0.153 = 0.240−0.087, 2.7% = (0.110−0.107)/0.110, +0.078 = 0.684−0.606.)
...
**3-g. 마무리 검사.** 4장 범위에서 `Phase [0-9]`·`Day [0-9]`·`freeze`·`구현 완료`·`make serve`·`24만`이 0건인지 확인한다. 4장 mermaid 블록은 건드리지 않는다.
  ...
  <acceptance_criteria>
    ...
    - 비교표 8행 유지: `echo "$R" | grep -cE '^\| (pop|cf|hybrid|hybrid_div) '` = 8, 그리고 `echo "$R" | grep -c '0.240'` ≥ 2(표 + 해석) · `grep -c '0.780'` = 1 · `grep -c '콘텍츠'` = 0
...
| D-05 | 감축 목표 3·4장 ≈2,500 · 5장 ≈1,500 | 08-03 Task 1 (계측·차순 삭제) |
