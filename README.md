...
- **Track B 데모 카탈로그** — 밀리의서재 공개 도서 페이지(sitemap 기반, 2026-09-xx 수집, 약 1,000권+). 비로그인 화면의 수치·메타·표지 URL만 저장하고 리뷰 텍스트·필명은 저장하지 않는다. 책 소개 텍스트는 TF-IDF 입력 전용으로 화면·API에 노출하지 않는다. 밀리의서재 요청 시 즉시 삭제한다. 수집 예절: 단일 스레드·요청 간 2.5초·식별 UA.
