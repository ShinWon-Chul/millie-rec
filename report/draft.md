...
## 1-2. 배경과 목표 [추천 목표]
메인의 역할은 다음에 읽을 책이 계속 나오는 경험을 만드는 것이고, 그 경험이 구독 유지로 이어진다. 밀리는 가입 시 7단계 취향 설정으로 사용자의 선호를 직접 받는다. 이 문서는 그 신호를 메인에서 어떻게, 얼마나 오래 살려 쓸지를 추천 목표로 정한다.
...
취향 설정과 행동 신호의 가중치가 증거가 쌓일수록 변해야 한다는 주장을 숫자로 보이려고 같은 모델을 온보딩 직후(n=0)와 이력 축적(n≥20)에서 따로 측정했다. n≥20에서 NDCG@10은 pop 0.087 대 hybrid 0.240으로 격차가 0.153이다. 이력 성분 β가 순위화에 들어온다는 주장을 이 숫자가 뒷받침하고 같은 상태의 cf 0.237과 hybrid의 차이는 0.003으로 작다. pop 기준선의 두 값 차이는 추천 제외 범위(n=0은 5권만, n≥20은 이력 전체)에서 온 바닥선이라 모델 성능과 무관하다.
