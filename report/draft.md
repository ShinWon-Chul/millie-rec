...
> ★ **시간 가변 가중치 — 증거가 쌓일수록 취향 설정의 비중이 내려간다.** 사용자 상태는 취향 설정·장기 행동·세션 세 성분의 가중합이고, 가중치는 그 사용자에게 쌓인 증거의 양과 최근성의 함수다(수식·상태표는 §2-2). 가입 직후엔 α=1로 온보딩 5권이 전부를 결정하고, 이력 n권이 쌓이면 α_n = max(0.2, 0.6·e^{−n/20})로 내려가며 나머지를 장기 행동과 세션이 나눈다. n≥20에서 NDCG@10이 pop 0.087 대 hybrid 0.240으로 격차 0.153이 난 것(§4-1 표)에서 이 설계가 숫자로 드러나고 같은 상태의 cf 0.237과의 차이는 작다.
