...
과제가 묻는 네 가지에 이 문서는 다음 한 문장씩으로 답한다.
...
## 1-4. 활용 데이터: 취향 설정 7단계는 전부 다른 종류의 신호다 (그림 1) [활용 데이터]
...
|---|---|---|---|
| ① 후보 생성 | 수십만 권 → 수백 권. 통로별로 다른 강점(행동, 콘텐츠, 인기, 신선도) | 정밀한 순위화는 비싸므로 먼저 좁힌다 | Recall@K |
...
| ④ 페이지 구성 | 행 선택, 순서, 행 간 중복 제거 | 메인은 Top-K 리스트가 아니라 **페이지**다. 재개와 탐색은 다른 목적 | 행 단위 QRS |

**후보 생성과 순위화의 분리**, **페이지 구성의 별도 단계화**는 대형 추천 서비스가 공개한 표준 구조다. 규모는 실측한 밀리 공개 카탈로그(약 9천 권)에 맞춰 Item-KNN(Item-based k-Nearest Neighbors: 함께 읽힌 책 기반 유사도) + 콘텐츠 유사도로 충분하다.
...
**구현 결정.** α/β/γ는 **사용자 벡터의 세 성분**(취향 설정, 장기 행동, 세션)의 가중치이고 채널(협업, 콘텐츠, 인기) 혼합비는 별개 상수다. 신규 사용자는 α=1에서 시작해 이력 n권이 쌓이면 α_n = max(0.2, 0.6×e^{−n/20})로 감쇠하며 나머지를 β:γ = 3:1로 나눈다. 응답의 `user_state_weights`가 이 값이라 화면의 혼합비와 실제 순위화가 일치한다.
...
## 4-1. Offline: 지표 3개가 구조 3단계와 1:1 (그림 4) [성능 평가]
